...
//******************************************************************************
  ...
  TCefCookieVisitorProc = {$IFDEF DELPHI12_UP} reference to {$ENDIF} function(
    ctx: Pointer; const name, value, domain, path: ustring; secure, httponly,
    hasExpires: Boolean; const creation, lastAccess, expires: TDateTime;
    count, total: Integer; out deleteCookie: Boolean): Boolean;
  ...
  ICefCookieManager = Interface(ICefBase)
    ...
    function VisitAllCookiesProc(const visitor: TCefCookieVisitorProc; context: Pointer): Boolean;
    ...
    function VisitUrlCookiesProc(const url: ustring; includeHttpOnly: Boolean;
      const visitor: TCefCookieVisitorProc; context: Pointer): Boolean;
...
{$ENDIF}
  ...
  public
    constructor Create(const _visitor: TCefCookieVisitorProc; _context: Pointer); reintroduce;
  end;
...
function CefIsMainProcess: Boolean;
...
begin
  if LibHandle <> 0 then
    ...
    if not Result and (LibHandle <> 0) then
    begin
...
function cefLoadFunctions: Boolean;
begin
  Result := False;
...
{$ENDIF}
  ...
  if Assigned(cef_string_wide_set) and
    ...
    Assigned(cef_drag_data_create) then
    Result := True
...
begin
  ...
  {$ENDIF}
    ...
    OutputDebugString(PChar('cef_execute_process returns ' + IntToStr(errcode)));
...
constructor TCefFastCookieVisitor.Create(const _visitor: TCefCookieVisitorProc; _context: Pointer);
begin
  inherited Create;
  FContext := _Context;
  FVisitor := _visitor;
...
begin
  Result := FVisitor(FContext, name, value, domain, path, secure, httponly, hasExpires,
...
function TCefCookieManagerRef.VisitAllCookiesProc(const visitor: TCefCookieVisitorProc;
  context: Pointer): Boolean;
begin
  Result := VisitAllCookies(
    TCefFastCookieVisitor.Create(visitor, context) as ICefCookieVisitor);
end;
